                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {user.roles.slice(0, 3).map((role, index) => (
                          <span
                            key={index}
                            className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-lg text-xs font-medium ${getRoleColor(role)}`}
                          >
                    ...
                    <Menu.Item key={index}>
                      {({ active }) => (
                        <motion.button
                          whileHover={{ x: 4 }}
                          onClick={item.action}
                          className={`w-full flex items-center px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm transition-all duration-200 ${
                            active 
...
const TopBar: React.FC<TopBarProps> = ({ sidebarExpanded, onLogout }) => {
  ...
  return (
    <header className="bg-white/80 z-50 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-700 h-16 sm:h-20 flex items-center justify-between px-3 sm:px-6 shadow-lg">
                  ...
                  <Menu.Item key={lang.code}>
                    {({ active }) => (
                      <motion.button
                        ...
                        className={`flex w-full items-center px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm transition-all duration-200 ${
                      ...
                      >
                        ...
                        {i18n.resolvedLanguage === lang.code && (
                          <SparklesIcon className="w-3 h-3 sm:w-4 sm:h-4 ml-auto" />
                        )}
                      </motion.button>
                    )}
                  </Menu.Item>
          ...
          {({ open }) => (
            <>
              <Popover.Button 
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="relative p-2 sm:p-3 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-all duration-200 shadow-md hover:shadow-lg"
              >
                ...
                <AnimatePresence>
                  {unreadCount > 0 && (
                    <motion.span
                      ...
                      animate={{ scale: 1 }}
                      exit={{ scale: 0 }}
                      className="absolute -top-1 -right-1 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-red-500 text-xs font-medium text-white shadow-lg"
                    >
                      {unreadCount}
                    </motion.span>
                  )}
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((notification, index) => (
                            <motion.div
                              ...
                              className={`p-3 sm:p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer ${
                            ...
                            >
                              <div className="flex items-start space-x-2 sm:space-x-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className={`text-xs sm:text-sm ${notification.unread ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'} line-clamp-2`}>
                                    {notification.text}
                                  </p>
