// components/dashboard/minerals/tantalum/TantalumPagination.tsx
import React from 'react';
import { useTranslation } from 'react-i18next';
import { ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/outline';

interface TantalumPaginationProps {
  currentPage: number;
  pageSize: number;
  totalItems: number;
...
const TantalumPagination: React.FC<TantalumPaginationProps> = ({
  currentPage,
  pageSize,
  totalItems,
  onPageChange,
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <span className="text-xs sm:text-sm text-gray-700 dark:text-gray-300">
              {t('tantalum.showing', 'Showing')} {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, totalItems)} {t('tantalum.of', 'of')} {totalItems}
            </span>
            <select
              value={pageSize}
...
export default TantalumPagination;