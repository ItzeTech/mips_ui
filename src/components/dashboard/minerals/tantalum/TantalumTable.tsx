...
const TantalumTable: React.FC<TantalumTableProps> = ({ tantalums, onView, onEdit }) => {
  ...
  const [selectedTantalumId, setSelectedTantalumId] = useState<string | null>(null);
  ...
  const handlePrintClick = (tantalumId: string) => {
    setSelectedTantalumId(tantalumId);
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-1">
                        <span className={`px-1.5 py-0.5 text-xs leading-5 font-medium rounded-md ${getStockStatusColor(tantalum.stock_status)}`}>
                          {t(`tantalum.status_${tantalum.stock_status}`, tantalum.stock_status) as string}
                        </span>
                        <span className={`px-1.5 py-0.5 text-xs leading-5 font-medium rounded-md ${getFinanceStatusColor(tantalum.finance_status)}`}>
                          {t(`tantalum.finance_${tantalum.finance_status}`, tantalum.finance_status) as string}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-1">
                        ...
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={() => handlePrintClick(tantalum.id)}
