// components/dashboard/minerals/tin/tinPrintModal.tsx
...
interface TinPrintModalProps {
  isOpen: boolean;
  onClose: () => void;
  tinId: string;
}

const TinPrintModal: React.FC<TinPrintModalProps> = ({ isOpen, onClose, tinId }) => {
  ...
  const handleViewReport = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get(`/reports/tin/${tinId}/payment-slip`, {
        responseType: 'blob'
      });
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `tin-report-${tinId}.pdf`);
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      >
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
          {t('tin.print_options', 'Print Options')}
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
          {t('tin.print_description', 'Choose how you would like to access the mineral report')}
        </p>
        ...
        <div className="flex flex-col space-y-3">
          ...
          >
            ...
            {t('tin.download_report', 'Download Report')}
          </motion.button>
