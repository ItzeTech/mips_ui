// components/dashboard/minerals/tungsten/TungstenPagination.tsx
import React from 'react';
import { useTranslation } from 'react-i18next';
import { ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/outline';

interface TungstenPaginationProps {
  currentPage: number;
  pageSize: number;
  totalItems: number;
...
const TungstenPagination: React.FC<TungstenPaginationProps> = ({ 
  currentPage, 
  pageSize, 
  totalItems, 
  onPageChange, 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <span className="text-xs sm:text-sm text-gray-700 dark:text-gray-300">
              {t('tungsten.showing', 'Showing')} {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, totalItems)} {t('tungsten.of', 'of')} {totalItems}
            </span>
            <select
              value={pageSize}
...
export default TungstenPagination;