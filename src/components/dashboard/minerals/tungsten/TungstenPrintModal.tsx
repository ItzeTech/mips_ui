// components/dashboard/minerals/tungsten/TungstenPrintModal.tsx
...
interface TungstenPrintModalProps {
  isOpen: boolean;
  onClose: () => void;
  tungstenId: string;
}

const TungstenPrintModal: React.FC<TungstenPrintModalProps> = ({ isOpen, onClose, tungstenId }) => {
  ...
  const handleViewReport = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get(`/reports/tungsten/${tungstenId}/payment-slip`, {
        responseType: 'blob'
      });
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `tungsten-report-${tungstenId}.pdf`);
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      >
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
          {t('tungsten.print_options', 'Print Options')}
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
          {t('tungsten.print_description', 'Choose how you would like to access the mineral report')}
        </p>
        ...
        <div className="flex flex-col space-y-3">
          ...
          >
            ...
            {t('tungsten.view_report', 'View Report')}
          </motion.button>
          ...
          >
            ...
            {t('tungsten.download_report', 'Download Report')}
          </motion.button>
...
export default TungstenPrintModal;