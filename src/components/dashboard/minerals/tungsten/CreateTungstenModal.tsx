// components/dashboard/minerals/tungsten/CreateTungstenModal.tsx
...
import { createTungsten, resetCreateStatus } from '../../../../features/minerals/tungstenSlice';
...
interface CreateTungstenModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const CreateTungstenModal: React.FC<CreateTungstenModalProps> = ({ isOpen, onClose }) => {
  const { t } = useTranslation();
  const dispatch = useDispatch<AppDispatch>();
  
  const { createStatus, error } = useSelector((state: RootState) => state.tungstens);
  ...
  useEffect(() => {
    if (createStatus === 'succeeded') {
      toast.success(t('tungsten.add_success', 'Tungsten added successfully'), {
        icon: '✅',
        style: {
          borderRadius: '10px',
          background: '#333',
        ...
        <>
          ...
          >
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-xs sm:max-w-md lg:max-w-lg max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
              {/* Header */}
              <HeaderCard
                title={t('tungsten.create_new', 'Add New Tungsten')}
                description={t('tungsten.create_description', 'Enter details to add a new tungsten record')}
                onClose={handleClose}
              />
              ...
              <form onSubmit={handleSubmit} className="p-3 sm:p-6 space-y-4 sm:space-y-6">
                {/* Supplier */}
                <SelectInput
                  label={t('tungsten.supplier', 'Supplier')}
                  icon={<UserIcon className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500" />}
                  ...
                  placeholder={t('tungsten.select_supplier')}
                />
                
                {/* Net Weight */}
                <TextInput
                  label={t('tungsten.net_weight', 'Net Weight (kg)')}
                  icon={<ScaleIcon className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500" />}
                ...
                {/* Date of Sampling */}
                <DateInput
                  label={t('tungsten.date_of_sampling', 'Date of Sampling')}
                ...
                <SubmitCancelButtons
                  ...
                  submitLabel={t('tungsten.create', 'Create Tungsten')}
...
export default CreateTungstenModal;